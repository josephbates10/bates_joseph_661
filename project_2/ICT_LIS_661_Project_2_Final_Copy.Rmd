...
library(tidyverse)
library(here)
World_Risk_Index <- read_csv(here("World Risk Index.csv"))
...
The data set provided (World_Risk_Index) was derived from the website Kaggle. The world Risk Report is an annual technical report on the global disaster risks and identifies the risk of an extreme natural event becoming a disaster for numerous countries worldwide. 
Tyago, M. (2022, September 7). World disaster risk dataset. Kaggle. Retrieved October 13, 2022, from
https://www.kaggle.com/datasets/trigg3rtrash/global-disaster-risk-index-time-series-dataset
...
My undergraduate degree is in Homeland Security, and I have spent substantial amount of my career in emergency management roles which has sparked my intereset in predictive analaysis in an emergency management index. I would also like to know if there are indicators within the data that predicts disaster susceptability which can be used by NGO's like US AID to allocate resources appropriately. The two Research Questions (RQs) that I would choose are: 
1) What top five countries are more susceptible and vulnerable to natural disasters?
2) What regions of the world have a higher disaster exposure score?
...
```{r}
ggplot(data = World_Risk_Index) +
  geom_point(mapping = aes(x = Region, y = WRI))
```
1) What I am trying to learn about the data? My intention was to illustrate a plot that shows what regions have the highest WRI. 
2) What kind of plot I chose? I chose a ggplot point for this project with the intention that regions will be depicted along the x axis and the WRI would be displayed along the y axix. 
3) What the plot is important for what I am trying to learn? This plot is important because it shows what regions have the highest WRI on the globe. 
4) What did the plot show about the data? This plot showed that most regions fall below the 20 WRI range, but that there are a few above the WRI range. 
